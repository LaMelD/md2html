...
- **Python 3.7 이상** (`dataclasses`, `from __future__ import annotations` 사용)
- **외부 의존성 없음** — 표준 라이브러리만 사용 (`argparse`, `html`, `os`, `re`, `sys`, `dataclasses`, `typing`)
- `requirements.txt` 는 CI/자동화 도구 호환을 위해 비어 있는 placeholder 다. `pip install` 단계가 필요 없다.
